...
static SemaphoreHandle_t Scan_sem = NULL;   //信号量，反正重复启动扫描
...
void WIFI_manager_Init(p_wifi_state_cb f)
{
    ...
    xSemaphoreGive(Scan_sem);       //创建信号量之后清空
...
esp_err_t WIFI_manager_AP(void)
{
   ...
   IP4_ADDR(&IPinfo.ip, 192,168,100,1);    //将IP地址填充到变量
...
static void Scan_Task(void * param)
{
    ...
    p_wifi_scan_cb scan_callback = (p_wifi_scan_cb)param;
    ...
    if(AP_Count > 0) {
        ...
        if(AP_List) {
            ...
            ESP_LOGI(TAG, "扫描到AP个数：%d",AP_Count);

            if(scan_callback)
            {
                scan_callback(AP_Count, AP_List);
            }
            free(AP_List);
        }
    } else {
...
esp_err_t WIFI_manager_scan(p_wifi_scan_cb f)
{
    ...
    if(pdTRUE == xSemaphoreTake(Scan_sem, 0))
    {
        ...
        //创建扫描任务（因为他是阻塞扫描的）
        return xTaskCreatePinnedToCore(Scan_Task, "Scan_Task", 8192, f, 3, NULL, 1);
    }
